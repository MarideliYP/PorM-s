...
from .forms import Producto_form, Inmueble_form, Oferta_form, Evento_form, Reserva_form
...
def vistaprincipal(request):
    ...
    if request.user.is_authenticated:
        ...
        context['reservas'] = reservas  # Ya es un queryset, sin comas

        # Manejo del carrito
        carritos = CarroCompra.objects.filter(usuario=request.user, pagado=False, enviado=False)
        if carritos.exists():
        ...
        enviados = CarroCompra.objects.filter(usuario=request.user, enviado=True, pagado=False)
...
@method_decorator(login_required, name='dispatch')
class Oferta_add(CreateView):
    template_name = 'oferta/add_oferta.html'
...
def editar_reserva(request, pk):
    ...
    # Si es GET, redirige (normalmente se usa con modal)
    messages.info(request, "Accediendo a la edición de reserva.")
    return redirect('index')
...
def eliminar_reserva(request, pk):
    ...
    if reserva.estado:
        messages.warning(
            request,
            "⚠️ No puedes eliminar una reserva ya confirmada."
        )
    else:
        ...
        messages.success(
            request,
            f"🗑️ Reserva eliminada con éxito para '{evento_nombre}'."
        )
...
def enviar_carrito(request):
    ...
    carrito.save()
    if carrito.enviado:
        messages.success(request, 'Su pedido fue enviado con éxito.')
    return redirect('index')
...
def pedir_cuenta(request):
    ...
    if carritos.last().cerrado:
        messages.success(request, f'Su cuenta fue enviada con éxito. El total es de ${suma}')
    return redirect('index')
